...
def render():
    ...
    while i < len(md):
        ...
        if s.strip().startswith("**Keywords**"):
            ...
            _add_run(ps, (
                "Sistemi za nadzor spletnih izpitov, ki temeljijo na umetni inteligenci, se "
                "večinoma zanašajo na neprekinjeno biometrično obdelavo, ki je v nasprotju z "
                "evropsko uredbo GDPR in maroškim zakonom 09-08. Predstavljamo večmodalno "
                "ogrodje, ki ohranja zasebnost in zaznava goljufanje brez shranjevanja "
                "biometričnih predlog, surovega videa ali jezikovne vsebine. Ogrodje povezuje "
                "YOLOv8-s za kontekstualno zaznavanje predmetov, MediaPipe FaceMesh za "
                "neidentificirajoče vedenjske namige in kodirnik Whisper-Base v načinu brez "
                "prepisa govora; izhodi modulov se združijo v model za izračun indeksa suma, "
                "izbran empirično med petimi različicami. YOLOv8-s doseže makro F1 0,606 na "
                "realni podmnožici COCO val2017, kodirnik Whisper pa na 400 posnetkih iz "
                "korpusa LibriSpeech ROC AUC 0,774 za prisotnost dodatnega govorca in 0,935 za "
                "zaznavanje šepetanja. Petkratno prečno preverjanje na 300 sejah z realnimi "
                "značilkami izbere naključni gozd kot najboljšo strategijo zlivanja "
                "(F1 = 0,905 ± 0,041), čeprav njegova prednost pred preprostim pravilom tehtane "
                "vsote ni statistično značilna (McNemar p = 0,37). Cevovod deluje s 23,7 "
                "oziroma 19,4 sličicami na sekundo (MPS oziroma CPU) na računalniku Apple M4 "
                "Max z največ 1.102 MB pomnilnika. Preverjanje zasebnosti s 30 resničnimi "
                "govorci pokaže, da napadalec s surovim zvokom doseže AUC 0,994 za ponovno "
                "identifikacijo, nezaščiteni zvočni metapodatki pa še vedno razkrijejo zmerno "
                "identiteto govorca (AUC 0,816), kar potrjuje, da brezvsebinski izhod ni "
                "samodejno tudi brez identitete. Objava metapodatkov prek diferencialno "
                "zasebnega mehanizma (Laplace, epsilon = 4 na vrednost) zniža ponovno "
                "identifikacijo na 0,547, blizu naključja, ob znižanju AUC zaznavanja z 0,774 "
                "na 0,625."
            ))
